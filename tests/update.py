...
fields = ['p_match','inns','bat','p_bat','team_bat','bowl','p_bowl','team_bowl',
          ...
          'inns_runs','inns_wkts','inns_balls','trail_by','lead_by','day','session','target']
...
def get_matids(min=current_year, max=current_year):
    matids = {}
    url = 'https://stats.espncricinfo.com/indian-domestic-2011/engine/records/team/match_results.html?class=1;id={};type=year'
    for y in range(min, max+1):
        ...
        for ele in d:
            ...
            if ' - ' in date:
                date, _ = date.split(' - ')
            elif '-' in date: # for date formats: Dec 10-11, 2017
    ...
    for inn in matdata.keys():
        for b in matdata[inn]:
            try:
                ...
                continue
            i = b['innings']
            trail = i.get('trailBy', None)
            lead = i.get('leadBy', None)
            day = i.get('day', None)
            session = i.get('session', None)
