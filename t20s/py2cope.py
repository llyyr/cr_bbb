...
def get_matids(min=current_year, max=current_year):
    matids = {}
    url = 'https://stats.espncricinfo.com/indian-domestic-2011/engine/records/team/match_results.html?class=6;id={};type=year'
...
def get_match_data(match_id):
    sid = 8098
    data = {}
    url = 'http://site.web.api.espn.com/apis/site/v2/sports/cricket/{}/playbyplay?contentorigin=espn&event={}&page={}&period={}&section=cricinfo'
    for inn in [1, 2]:
        items = []
        for i in range(1, 7):
            while True:
                r = req.get(url.format(sid, match_id, i, inn)).json()
                if 'commentary' in r: 
                    items = items + r['commentary']['items']
                    if items == []:
                        return
                    break
                else:
...
def add_metadata(df, mat):
    ...
    if 'Blast' in comp:
        comp = 'T20 Blast'
    elif 'Indian Premier League' in comp:
        comp = 'IPL'
    elif 'Pakistan Super League' in comp:
        comp = 'PSL'
    elif 'Caribbean Premier' in comp:
        comp = 'CPL'
    ...
    matchinfo.loc[mat] = [mat, date, year, ground, country, winner, toss_winner, comp]
...
def main():
    ...
    if os.path.isfile(file):
        ...
        matids = get_matids(2015, current_year).keys() # start to end (inclusive)
    to_fetch = len(matids)
